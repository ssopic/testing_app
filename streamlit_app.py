...
class MapReduceEngine:
    ...
    def _create_batches(self, docs: List[dict]) -> List[List[dict]]:
        ...
        1. Small docs are 'bin-packed' into batches.
        2. Giant docs are split into chunks with overlap.
        """
        batches = []
        current_batch = []
        current_chars = 0
        ...
        for doc in docs:
            ...
            # --- CHANGE START: Proactive Splitting Logic ---
            # If a single doc is larger than the batch limit, we split it.
            if doc_len > BATCH_SIZE_CHARS:
                ...
                # We simply chunk it by BATCH_SIZE_CHARS with overlap
                start = 0
                while start < doc_len:
                    end = min(start + BATCH_SIZE_CHARS, doc_len)
                    chunk_text = doc_body[start:end]
                    
                    # We pass the same 'Bates_Identity' so the reducer knows they are the same doc
                    # The Map Agent will just see "Document Content" and extract facts.
                    chunk_doc = {
                        "Bates_Identity": doc_id, 
                        "Body": chunk_text
                    }
                    # Giant docs get their own batch (containing just one chunk)
                    ...
                    # Overlap logic: Move start back by OVERLAP_CHARS for the next chunk
                    start = end - OVERLAP_CHARS
                    
            # --- CHANGE END ---
            
            # --- OLD LOGIC (Preserved for small docs) ---
            # If doc fits in current batch, add it.
    ...
    def map_document_batch(self, batch_docs: List[dict], goal: ExtractionGoal) -> List[DocumentFact]:
        ...
        for d in batch_docs:
            ...
            # --- CHANGE: Removed Truncation ---
            # d_body = d.get('Body', '')[:20000] <-- OLD
            d_body = d.get('Body', '') # New: We rely on _create_batches to handle size
    ...
    def reduce_facts(self, facts: List[DocumentFact], user_query: str) -> str:
        ...
        # --- CHANGE START: Citation Clutter Fix (Aggregation) ---
        # Logic: Group facts by Doc ID before creating the context string.
        # This prevents "Source (Doc A)... Source (Doc A)..." repetitions from split chunks.
        
        merged_facts: Dict[str, Dict] = {} # Key: doc_id, Value: {summary: [], quotes: []}
        ...
        for doc_id, data in merged_facts.items():
            # Join multiple summaries for the same doc with a separator
    ...
    def run_parallel_map(self, docs: List[dict], goal: ExtractionGoal, status_container=None) -> List[DocumentFact]:
        ...
        if status_container:
            status_container.write(f"📦 Optimized {len(docs)} documents into {len(batches)} processing batches (including splits).")
