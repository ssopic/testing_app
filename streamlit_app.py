...
class MapReduceEngine:
    ...
    def map_document_batch(self, batch_docs: List[dict], goal: ExtractionGoal) -> List[DocumentFact]:
        ...
        prompt = ChatPromptTemplate.from_template(
            """You are a Fact Extraction Agent analyzing a batch of documents.
        ...
        try:
            ...
            # Validation: Ensure we didn't hallucinate IDs
            valid_results = []
            for item in result.results:
                # Fuzzy match or direct check if ID is known
                # For now, we accept it if it looks reasonable
                valid_results.append(item)
                
            return valid_results
            
        except Exception as e:
            # Fallback: Return empty results for these docs so pipeline continues
