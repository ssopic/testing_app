...
SYSTEM_PROMPTS = {
    ...
    "Grounding Agent": (
        ...
        "RULES:\n"
        "1. LABELS: You MUST treat all node labels as UPPERCASE (e.g., :PERSON, :ORGANIZATION, :LOCATION).\n"
        "2. FILTERING RESTRICTION (CRITICAL): You may ONLY apply filters/properties to nodes labeled :PERSON.\n"
        "   - For :PERSON nodes, ONLY use the property '.name'.\n"
        "   - For ALL OTHER nodes (Location, Organization, etc.), do NOT use any properties or WHERE clauses. Ignore attributes like 'city', 'type', etc.\n"
        "3. CHAIN OF THOUGHT:\n"
        "   - Identify Entities. If it's a Person, map to :PERSON {name: ...}.\n"
        "   - If it's a Location/Org, map to :LOCATION or :ORGANIZATION with NO properties.\n"
        "   - Check Relationship Types against Schema.\n"
        ...
        "PROVENANCE: Return provenance. Use `coalesce(r.source_pks)` for relationships and `coalesce(d.doc_id)` for 'MENTIONED_IN' targets.\n"
...
#        "CONSTRAINT RULE: Do NOT use properties in the WHERE clause that are not listed in the Schema's NodeProperties."
    ...
    "Cypher Generator": (
        ...
        "RULES:\n"
        "1. PATHS: Continuous paths only. No self-loops. Assign variables to all relationships.\n"
        "   Example: `(a:PERSON)-[r1:TYPE]->(b:LOCATION)`.\n"
        "2. LABELS: ALL LABELS MUST BE UPPERCASE (e.g., :PERSON, :LOCATION, :ORGANIZATION).\n"
        "3. PROPERTIES (STRICT): \n"
        "   - For (:PERSON), ONLY use `.name`. Example: `toLower(n.name) CONTAINS '...'`.\n"
        "   - For ALL OTHER LABELS, DO NOT use any properties. DO NOT filter by name, id, or type.\n"
        "4. FUZZY MATCHING: Only applied to :PERSON.name.\n"
        "5. VERB FILTERS: If 'filter_on_verbs' is provided, check `raw_verbs` on relationships.\n"
        "6. PROVENANCE (CRITICAL): You MUST extract the PKs.\n"
        "   - If the relationship [r] has the property `source_pks`, use: `UNWIND r.source_pks AS pk`.\n"
        "   - If the node is a :Document, use its `.doc_id`.\n"
        "   - General Fallback Pattern: `UNWIND (CASE WHEN r.source_pks IS NOT NULL THEN r.source_pks ELSE [coalesce(target.doc_id, source.doc_id)] END) AS pk`.\n"
        "7. DISTINCT: Always use `RETURN collect(DISTINCT pk) as source_document_ids`.\n"
